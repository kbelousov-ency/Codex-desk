...
export default function ChangeTurnPicker({ value, turns, unknown, active, onChange }: {
  ...
  return <div className="change-turn-picker" onBlur={event => {
    ...
    <button ref={trigger} type="button" className="change-turn-trigger" aria-label="Изменения по запросу" aria-haspopup="dialog" aria-expanded={visible} aria-controls={visible ? `${id}-menu` : undefined} data-value={value} data-tooltip={`${current.title} · ${current.description}`} onClick={() => visible ? close() : show()} onKeyDown={event => {
