...
export default function App({ bridge = window.codex, sessionId = 'default', active = true, initialThread, initialFork, initialDraft = '', initialAttachments = [], initialQueue, initialPendingMessage, initialScrollTop, initialScrollAnchor, initialPreservedDraft, restoreSettings, workspace, jump }: {
  ...
  captureUpdateRef.current = persistent => {
    if (!persistent && (sending || queue.inFlight || readingImages || selectingFiles || editingMessage || loadingEdit || showSettings || showExport || showHandoff || showChangesReview || Boolean(fileViewer) || confirmFull || codex.loading || codex.busy || codex.terminalOpen || codex.requests.length || codex.connection === 'connecting')) return null;
    ...
    <main className="main-column">
      ...
      <div className="chat-scroll" ref={chatRef} onScroll={e => { const el = e.currentTarget; if (!active || pendingScroll.current !== undefined) return; savedScrollTop.current = el.scrollTop; savedAnchor.current = readScrollAnchor(el); setScrolledUp(el.scrollHeight - el.scrollTop - el.clientHeight > 100); workspace?.onSessionStateChange?.(sessionId); }}>
        {!showChatSearch && !chatItems.length && !codex.loading && !codex.busy && !codex.items.some(item => activityLabel(item)) ? <section className="welcome">
          ...
          <ChatSearch key={codex.thread?.id || 'new'} items={codex.items} turnWork={codex.turnWork} open={showChatSearch} active={active} onClose={() => { setShowChatSearch(false); inputRef.current?.focus({ preventScroll: true }); }} hasEarlier={Boolean(codex.itemCursor)} loading={codex.loading} onLoadEarlier={() => void codex.loadEarlier()} onEditMessage={editMessage} onBookmark={saveBookmark} onAnswerQuestion={answerQuestion} questionDisabled={questionDisabled} editDisabled={sending || readingImages || loadingEdit || codex.loading || codex.terminalOpen || Boolean(workspace?.actionBusy)} />
      ...
      <div className="composer-area">
        ...
        <div ref={composerRef} className={`composer ${dragOver ? 'drag-over' : ''} ${queue.state.items.length ? 'with-queue' : ''}`} onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setDragOver(false); }} onDrop={e => { e.preventDefault(); setDragOver(false); void addImages([...e.dataTransfer.files]); }}>
