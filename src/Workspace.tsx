...
import { Archive, FolderPlus, LoaderCircle, MessageSquare, Plus, Terminal, X } from 'lucide-react';
...
import type { Attachment, CodexBridge, SessionInfo, Settings, Thread, ThreadAction, UpdateStatus, UpdateTabSnapshot } from './types';
import './nightly-update.css';

type Tab = SessionInfo & { bridge?: CodexBridge; initialThread?: Thread; archivedThread?: Thread; draft?: string; attachments?: Attachment[]; restoreSettings?: Settings };
...
function TabbedWorkspace() {
  ...
  const updateCaptures = useRef(new Map<string, () => UpdateTabSnapshot | null>());
  ...
  const updateState = useRef({ activeId, starting, opening, closing, confirmClose, actionDialog });
  updateState.current = { activeId, starting, opening, closing, confirmClose, actionDialog };
  const registerUpdateCapture = useCallback((id: string, capture: () => UpdateTabSnapshot | null) => {
  ...
  useEffect(() => {
    ...
    const removePrepare = window.codex.onUpdatePrepare(({ requestId }) => {
      ...
      const prepare = async () => {
        ...
        if (current.starting || current.opening || current.closing || current.confirmClose || current.actionDialog || actionPending.current || pendingOpen.current) {
        ...
        for (const tab of tabsRef.current) {
          const snapshot = tab.archivedThread
            ? { archivedThread: { id: tab.archivedThread.id, cwd: tab.cwd, name: tab.archivedThread.name }, draft: '', attachments: [] }
            : updateCaptures.current.get(tab.id)?.();
          if (!snapshot) {
            await window.codex.completeUpdatePrepare({ requestId, defer: true });
            updateRequest.current = null;
            return;
    ...
    return () => { removeStatus(); removePrepare(); };
  }, []);
  ...
  useEffect(() => {
    ...
    void window.codex.completeUpdateRestore?.().catch(() => setError('Не удалось подтвердить восстановление вкладок после обновления.'));
  }, [starting, tabs]);
  ...
  const activate = (id: string) => { setActiveId(id); setError(''); };
  ...
  const controls: WorkspaceControls = {
    projects, opening: opening || actionBusy || preparingUpdate, expanded, histories, threadNames, searchRevision,
    archiveOpen, archiveThreads, archiveLoading, archiveError, archiveCursor, actionBusy: actionBusy || preparingUpdate,
    ...
    newChat: cwd => void open(cwd), openThread: (cwd, thread) => void open(cwd, thread), report, registerUpdateCapture,
  };

  return <div className="workspace-shell" ref={shellRef}>
    <div className="workspace-tabs-bar">
      <div className="session-tabs" role="tablist" aria-label="Открытые диалоги">
        {tabs.map(tab => {
          ...
          return <div className={`session-tab ${activeId === tab.id ? 'active' : ''}`} data-session-id={tab.id} key={tab.id}>
    ...
    <div className="workspace-views">
      {tabs.map(tab => <div className="session-view" id={`view-${tab.id}`} role="tabpanel" aria-labelledby={`tab-${tab.id}`} data-session-id={tab.id} hidden={activeId !== tab.id} key={tab.id}>
        {tab.archivedThread ? <ArchiveView thread={tab.archivedThread} active={activeId === tab.id} workspace={controls} /> : <App bridge={tab.bridge} sessionId={tab.id} active={activeId === tab.id} initialThread={tab.initialThread} initialDraft={tab.draft} initialAttachments={tab.attachments} restoreSettings={tab.restoreSettings} workspace={controls} />}
      </div>)}
    ...
    {!preparingUpdate && updateStatus && <div className={`nightly-update-status ${updateStatus.state === 'error' ? 'is-error' : ''}`} role="status"><span>{updateStatus.message || (updateStatus.state === 'error' ? 'Не удалось применить обновление. Приложение продолжает работать.' : 'Обновление Nightly готово. Перезапуск после завершения работы.')}</span>{updateStatus.state === 'error' && <button type="button" className="icon-button small" aria-label="Скрыть сообщение об обновлении" onClick={() => setUpdateStatus(null)}><X size={14} /></button>}</div>}
