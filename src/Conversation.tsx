...
import { Check, Copy, ImagePlus, Terminal } from 'lucide-react';
...
function Conversation({ items, turnWork, searchable = false }: { items: Item[]; turnWork: Record<string, TurnWork>; searchable?: boolean }) {
  const entries = useMemo(() => conversationEntries(items), [items]);
  return <>{entries.map(entry => entry.type === 'message'
    ? <Message key={entry.key} item={entry.item} />
...
function Message({ item }: { item: Item }) {
  ...
  return <article className={`message ${user ? 'user-message' : 'assistant-message'}`} data-item-id={item.id}>
    <div className="message-label"><span className={`message-avatar ${user ? 'user-avatar' : ''}`}>{user ? <PixelAvatar /> : <Terminal size={13} />}</span><strong>{user ? 'Вы' : 'Codex'}</strong>{!user && content && <button className="icon-button copy-button" aria-label="Скопировать сообщение" title="Скопировать сообщение" onClick={() => { void navigator.clipboard.writeText(content).then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500); }); }}>{copied ? <Check size={13} /> : <Copy size={13} />}</button>}</div>
