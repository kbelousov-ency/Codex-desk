...
try {
  ...
  await withReleaseLock(root, async () => {
    ...
    await writeFile(path.join(next, 'README.txt'), '\uFEFF' + [
      ...
      'Для выбранного агента нужен его установленный и настроенный CLI: Codex CLI или Claude Code CLI.',
      'Войдите в свой аккаунт CLI или используйте корпоративную конфигурацию на компьютере получателя.',
      ...
      'Этот же exe подходит для первой установки и обновления существующего Release.',
