...
try {
  ...
  await withReleaseLock(root, async () => {
    ...
    await writeFile(path.join(next, 'README.txt'), '\uFEFF' + [
      ...
      'Для корпоративного роутера нажмите «Подключить через браузер», подтвердите код на портале и примените настройки в мастере. Нужен доступ к https://coder-portal.encycam.com.',
      'Скачивать config.toml не требуется. Перед сохранением создаётся резервная копия; остальные настройки, профили и MCP сохраняются.',
      'Можно оставить текущую конфигурацию или импортировать готовый файл; ручной импорт заменяет config.toml целиком после подтверждения.',
