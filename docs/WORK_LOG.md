...
## Прогресс после уточнений — 2026-09-22

По снимку пользователя подтверждён случай: асинхронный вопрос Codex пришёл как `agentMessage` с `phase:final_answer`, `delivery:async`, ответ пользователя и дальнейшие reasoning/команды/правки сохранили тот же `turnId`. События продолжались, но прежняя группировка всех items по одному turn переносила их в свёрнутый блок перед вопросом. `answerStartedAt` также ошибочно считался концом любой последующей работы этого turn.

`conversationEntries` теперь разделяет ход работы по пользовательским сообщениям, структурированным вопросам и сообщениям с `delivery:async`, сохраняя исходный `turnId`. Продолжение показывается после соответствующего ответа; его сворачивание определяется собственным итоговым сообщением или завершением turn. Обычные финальные ответы и поздние события инструментов сохраняют прежнюю группировку. Предыдущие части доступны для раскрытия; незавершённые инструменты сохраняют индикаторы. Время относится ко всему серверному turn и показывается только в последнем блоке, длительности отдельных уточнений не выдумываются. `answerItemId` связывает живой `answerStartedAt` с конкретным итоговым item.

Поздний user echo заменяет оптимистичное сообщение на прежнем месте, чтобы уже пришедшие действия не оказывались перед ответом. `localMessageId` сохраняет только ключ отображения для старого сервера без `clientId`; он не используется как подтверждение доставки. Настройки Codex, запросы модели, receipt matching и история на диске не меняются.

Проверка `tests/conversation-items.test.mjs` покрывает обычные/повторные уточнения, async-вопросы, историю без turnId/phase, ключи echo и поздние события предыдущего turn. Расширенный `test:agent-questions` воспроизводил ошибку на прежнем dist; проверяет видимость reasoning/commentary/команд после steer и idle send, повторный final-вопрос в том же ходе, late echo, сохранение раскрытых инструментов и `requestUserInput/respond`. Это подставные события production renderer, без запросов модели. Прежние `test:work-log`, `test:work-collapse`, `test:message-queue` прошли.
